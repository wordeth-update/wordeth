...
**Follow-up.** Three worlds double as levels tied to Lyric IQ tier and to the live Streak count, so progress is visible as a change of place rather than a number. The first pass drew them in CSS (meadow, city, desert). After a design round they were redrawn as hip-hop street scenes (the block, the court, the rooftop) in SVG: richer than CSS shapes can get, still vector and tiny (12–30 KB each), and inlined at runtime rather than loaded as images so the Unbounded display font applies to the signs and graffiti. The UI palette moved with them to cream cards, near-black ink and amber / mint / hot pink accents.
