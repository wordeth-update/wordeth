...
- **Pages**: `index.html` (home), `verses.html` (audio rooms), `lyrics.html` (lyric search), `merch.html` (merchandise), `articles.html`, `signin.html`, `signup.html`, `profile.html`, `partner-upload.html` (label setup: artist roster CSV + template artwork upload), plus admin pages for ads and analytics
...
- **API Routes**: Modular route files in `/routes/` directory:
  ...
  - `/api/partner` — label partner dashboards (auth, summary, artist drill-down, SKU views, geo heatmaps, shareable links, bulk CSV roster upload, template artwork upload/management via object storage)
