...
- **Verses (Audio Rooms)**: Uses Agora RTC SDK in `live` mode (all-host strategy) for scalable audio/video, with Socket.io for room management. Supports server-side token generation, Web Audio API for filters, and publish/unpublish for role transitions. Includes listener-first stage access with promotion paths and multi-person video grid. Full room UI CSS in `verses.css` covering: modals (create room, music share, add users, replay), room controls toolbar, chat section, host controls panel, shared audio player overlay, and responsive mobile layout.
  - **Agora Mode**: `live` mode with all-host strategy — every user (including listeners) joins as `host` role with PUBLISHER tokens for reliable bi-directional audio. `setClientRole('host')` called once on join, never again. Listeners join with `skipPublish: true` so they hear but don't transmit. Promotion calls `publishAgoraAudio()` in-place; demotion calls `unpublishAgoraAudio()`. No role switching for transitions. Scales to 1000+ listeners with few active speakers.
  - **Mobile Toolbar**: Secondary buttons (Effects, Karaoke, Camera, Photo) hidden on mobile via CSS to keep toolbar clean; accessible through host controls panel instead.
  - **Event Deduplication**: `room-joined` handler gates on `_agoraJoinHandled && _joinConfirmed` to skip duplicate processing. `host-changed` only fires toast when transitioning from non-host. SFX debounced (800ms per sound name, 1s for join sound). Walkthrough guarded by `_walkthroughActive` flag.
