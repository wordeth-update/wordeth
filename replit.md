...
- **Verses (Audio Rooms)**: Uses Agora RTC SFU (Selective Forwarding Unit) for scalable audio/video transport via Agora Web SDK v4.22 loaded from CDN, replacing the previous WebRTC peer-to-peer mesh. Socket.io remains for room management, chat, invites, karaoke sync, and participant events. Agora client uses `live` mode with `host` role (speakers/broadcasters who publish) and `audience` role (listeners who subscribe only). Token generation is server-side via `/api/agora/token`. Audio tracks use `AgoraRTC.createCustomAudioTrack()` fed by Web Audio API for voice filters and YouTube/music mixing. Video uses `AgoraRTC.createCustomVideoTrack()` for camera, AR filters, and karaoke video. Role switching (speaker promotion/crowd demotion) dynamically changes Agora client role and publishes/unpublishes tracks. Reconnection on visibility/background change detects Agora connection state and rejoins if needed. Hosts can kick participants (move to crowd or remove entirely) or invite listeners to stage via a three-dot menu on participant avatars. Multi-person video grid supports up to 6 simultaneous feeds. Host controls video mode (Off/Ask/Open), Mute All, and Close Room. Photo/image sharing via chat allows photo album selection on mobile.
