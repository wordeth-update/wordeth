...
- **Verses (Audio Rooms)**: Real-time WebRTC peer-to-peer audio via Socket.io signaling server. Web Audio API for voice filters and audio mixing (mic + YouTube). YouTube embed integration for karaoke with "Share Audio" button that captures tab audio and mixes it into the outgoing WebRTC stream. MediaRecorder for performance recording. Mobile share modal with camera stream, photo share (up to 10MB), music file sharing, and native screen capture options.
  - **Music File Sharing**: Users can share MP3, WAV, M4A, AAC, OGG, FLAC files (up to 50MB) in audio rooms. Files uploaded via `/api/rooms/upload-audio` (multer), stored in `uploads/audio/` with auto-cleanup after 24 hours. Shared via `room-audio-share` socket event. Full audio player overlay with play/pause/seek/volume controls. Audio messages appear as clickable cards in room chat. Drag-and-drop upload with progress bar. Auto-parses filename for artist/title.
