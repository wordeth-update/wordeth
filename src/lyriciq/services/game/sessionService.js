...
async function createSession(player, { gameMode = 'QUICK_PLAY', category = null, challengeCode = null, artist = null } = {}) {
    ...
    if (artist && (artist.providerArtistId || artist.key || artist.name)) {
        ...
        if (!resolved || resolved.playable < config.catalog.minArtistTracks) {
            ...
            const why = rep ? ` (${resolved.playable} usable; provider gave ${rep.id ?? '–'} by id, ${rep.name ?? '–'} by name${rep.learnedId ? ', real id ' + rep.learnedId : ''}${rep.probes && rep.probes.length ? '; tried ' + rep.probes.join(' ') : ''}${rep.errors.length ? '; ' + rep.errors.join(', ') : ''}${rep.rejected && rep.rejected.length ? '; skipped e.g. ' + rep.rejected.map((r) => '“' + r + '”').join(', ') : ''})` : '';
