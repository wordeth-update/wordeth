...
## Stage 28 — Share cards
**Built.** The result is the ad. At completion the session stores a public snapshot (`shareCard`: first name or "Someone on Wordeth", Lyric IQ, provisional flag, the results-screen voice line, the session headline such as "Rapid Fire · 14 in 60s", top genre IQ, daily streak, world tier). `GET /s/:id` serves a landing page with Open Graph / Twitter tags, the card and a "Take the test" link carrying the challenge id; `GET /s/:id/card.png` renders the card with headless Chromium on the player's world scene (the game's own SVG), 1200×630 for link previews and `?format=story` 1080×1920 for Stories, cached in memory and served immutable. The results screen shows the card and offers Share (hands the PNG itself to the share sheet where `navigator.canShare({ files })` allows, so it lands in iMessage / Instagram as an image), Save for Stories (download of the tall cut) and Copy link. Both routes strip CSP / COOP / CORP headers so crawlers and embeds can fetch them. Card copy is server-side and mirrors the client's Lyric IQ lines; nothing on a card or share page is lyric text.
**Design.** Streets palette on the tier's scene with a directional shade for legibility, gradient display numeral (mint → amber → orange) with ink drop shadow, cream voice-line card, amber / neon-ink / cream chips, mint URL pill. Fonts load from Google; the card degrades to the fallback face if they are unreachable, and production has network.
**Tests.** `integration/share.test.js`: snapshot contents for users and guests, share links in results, OG/Twitter tags, no lyric leakage, 404 for unknown / malformed / unfinished sessions, PNG dimensions for both formats and cache hit (skipped where no Chromium is available), and template assertions.
